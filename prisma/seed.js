...
function truncate(text, max = 191) {
    ...
    const usuarios = [
        {
            ...
            senha: 'admin123',
            role: 'admin',
            descricaoRole: 'Responsável por gerenciar todo o sistema, incluindo usuários, permissões, configurações globais e dados operacionais. Possui acesso irrestrito a todos os recursos da plataforma.',
        ...
        },
        {
            ...
            senha: 'admin123',
            role: 'consultor',
            descricaoRole: 'Responsável por realizar vendas, prestar atendimento aos clientes e auxiliar na operação da plataforma. Atua como ponto de contato direto com os clientes, orientando sobre produtos, serviços e funcionalidades disponíveis.',
        ...
        },
        {
            ...
            role: 'subordinado',
            descricaoRole: 'Usuário vinculado a uma empresa cliente, com acesso limitado para realizar ações em nome da organização. Pode visualizar e solicitar remessas, consultar entregas, e interagir com funcionalidades autorizadas pela empresa principal.',
    ...
    const enderecos = [
        {
            ...
            estado: 'MG',
            cep: '30130-000',
            tipoEndereco: client_1.TipoEndereco.residencial,
...
async function seedTiposEntrega() {
    const tipos = [
        client_1.TipoEntregaValor.balcao,
        client_1.TipoEntregaValor.correios,
        client_1.TipoEntregaValor.motoboy_letscom,
        client_1.TipoEntregaValor.outros,
    ];
...
async function seedTipoEntregaUser() {
    ...
    const balcao = await prisma.tipoEntrega.findFirst({
        where: { tipo: client_1.TipoEntregaValor.balcao },
    });
    const correios = await prisma.tipoEntrega.findFirst({
        where: { tipo: client_1.TipoEntregaValor.correios },
    });
    if (!balcao || !correios)
        return;
...
async function seedModelosTecnicos() {
    const cliente = (await prisma.user.findUnique({ where: { email: 'wltech2022@gmail.com' } })) ??
        (await prisma.user.findFirst({ orderBy: { id: 'asc' } }));
    if (!cliente) {
        console.warn('ModelosTecnicosSeeder: nenhum usuário encontrado.');
        return;
    }
    const produto = (await prisma.product.findFirst({ where: { nome: 'SEM CHIP' } })) ??
        (await prisma.product.findFirst({ orderBy: { id: 'asc' } }));
    const tecnologia = (await prisma.tecnologia.findUnique({ where: { nome: 'Nenhuma Tecnologia' } })) ??
...
async function requireModelo(nomeModelo) {
